...
exports.commands = {
	...
	requesthelp: function (target, room, user) {
		...
		saveReports();
		Core.pmUpperStaff('A new report has been submitted by ' + user.name + '. ID: ' + reportId + ' Message: ' + target.trim(), '~Server');
		Rooms('upperstaff').add('A new report has been submitted by ' + user.name + '. ID: ' + reportId + ' Message: ' + target.trim()).update();
	...
	reports: function (target, room, user, connection) {
		...
		case 'view':
			...
			for (let u in Object.keys(Reports)) {
				...
				output += '<tr><td>' + currentReport.id + '</td><td>' + Chat.escapeHTML(currentReport.reporter) + '</td><td>' +
					Chat.escapeHTML(currentReport.message) + '</td><td>' + date + ' </td><td>' + (currentReport.status === 'Pending Staff' ? '<font color=blue>Pending Staff</font>' : (~currentReport.status.indexOf('Accepted by') ? '<font color=green>' + Chat.escapeHTML(currentReport.status) + '</font>' : Chat.escapeHTML(currentReport.status))) + '</td></tr>';
			}
		...
		case 'accept':
			...
			Core.pmUpperStaff(user.name + " accepted the report by " + Reports[id].reporter + ". (ID: " + id + ")", '~Server');
			Rooms('upperstaff').add(user.name + " accepted the report by " + Reports[id].reporter + ". (ID: " + id + ")").update();
			break;
		case 'decline':
		case 'deny':
			...
			Core.pmUpperStaff(user.name + " denied the report by " + Reports[reportid].reporter + ". (ID: " + reportid + ")", '~Server');
			Rooms('upperstaff').add(user.name + " denied the report by " + Reports[reportid].reporter + ". (ID: " + reportid + ")").update();
		...
		case 'delete':
			...
			Core.pmUpperStaff(user.name + " deleted the report by " + Reports[rid].reporter + ". (ID: " + rid + ")", '~Server');
			Rooms('upperstaff').add(user.name + " deleted the report by " + Reports[rid].reporter + ". (ID: " + rid + ")").update();
			delete Reports[rid];
			saveReports();
